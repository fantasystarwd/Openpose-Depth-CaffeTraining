    ...
    with open(data_file, 'r') as f:
        for line in f:
            ...
            if len(line) > 0:
                if line[0] != '#':
                    fields = line.split(",")
                    iter = float(fields[0].strip())
                    if iter % 5000 == 0:
					    data[0].append(iter)
					    data[1].append(float(fields[5].strip()))
						#data[1].append(float(fields[3].strip())+float(fields[4].strip())+float(fields[5].strip())+float(fields[6].strip()))
    return data
